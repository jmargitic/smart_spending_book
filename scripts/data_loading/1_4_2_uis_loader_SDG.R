...
url<-'https://uis.unesco.org/sites/default/files/documents/bdds/022024/SDG.zip'
...
destfile <- str_c(fldr_uis,'/raw/SDG.zip')
...
sdg_n<-read_csv(str_c(fldr_uis,'/raw/SDG_DATA_NATIONAL.csv')) |> 
...
sdg_r<-read_csv(str_c(fldr_uis,'/raw/SDG_DATA_REGIONAL.csv'))|> 
  rename(indicator = indicator_id,
         isoalpha3 = region_id) |> 
  # Adding source
  mutate(source = 'UNESCO-UIS')
...
sdg_dic<-read_csv(str_c(fldr_uis,'/raw/SDG_LABEL.csv')) |> 
  rename(indicator = INDICATOR_ID,
